...
export default function GamePlayer(props){
    ...
    return <div key={player.id} className="p-2">
        <div key={player.id} className="border p-3 w-100 d-flex justify-content-between">
            ...
            <div className="d-flex justify-content-center align-items-center">
                {!game.finishTime && game.activePlayer && game.activePlayer.id === player.id && (player.id===myId ?  <span className="alert alert-success">My turn</span> : <span>Players turn</span>)}
                {winner && <WinnerSign/>}
            </div>
        </div>
